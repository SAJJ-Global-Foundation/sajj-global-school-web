        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `block px-4 py-2.5 text-sm font-medium transition-colors ${
              isActive ? 'text-navy-700 bg-brand-light' : 'text-brand-muted hover:text-navy-700 hover:bg-brand-light'
            }`
          }
        >
          {item.label}
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-18">
            {/* Logo lockup */}
            <Link to="/" className="flex items-center gap-3 flex-shrink-0 group" aria-label="Sajj Global School — Home">
              <img
                src={sgsLogo}
                alt="SGS Crest"
              ...
              <div className="hidden sm:block leading-tight">
                <p className="font-display font-bold text-navy-900 text-base leading-none">Sajj Global School</p>
                <p className="text-xs text-brand-muted font-medium tracking-wide">Excellence in Education</p>
              </div>
            </Link>
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-brand-border">
              <Link to="/" onClick={() => setMobileOpen(false)} className="flex items-center gap-2">
                ...
                <span className="font-display font-bold text-navy-900 text-sm">Sajj Global School</span>
              </Link>
              <button onClick={() => setMobileOpen(false)} className="p-1.5 rounded-lg text-brand-muted hover:bg-brand-light">
                <X size={18} />
              </button>
            </div>
                              ...
                              <NavLink
                                ...
                                className={({ isActive }) =>
                                  `block px-3 py-2.5 rounded-lg text-sm transition-colors ${
                                    isActive ? 'text-navy-700 bg-brand-light font-semibold' : 'text-brand-muted hover:text-navy-700 hover:bg-brand-light'
                                  }`
                                }
                              >
                                {child.label}
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `block px-3 py-3 rounded-xl text-sm font-medium transition-colors ${
                            isActive ? 'text-navy-700 bg-brand-light font-bold' : 'text-brand-muted hover:text-navy-700 hover:bg-brand-light'
                          }`
                        }
                      >
                        {item.label}
            ...
            <div className="p-4 border-t border-brand-border">
              <Button to="/admissions#enquiry" variant="gradient" size="md" className="w-full" onClick={() => setMobileOpen(false)}>
                Admission Enquiry
              </Button>
            </div>
          </nav>
